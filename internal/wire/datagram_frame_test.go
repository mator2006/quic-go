...
var _ = Describe("STREAM frame", func() {
	Context("when parsing", func() {
		It("parses a frame with flow ID and length", func() {
			data := []byte{0x30 ^ 0x2 ^ 0x1}
			data = append(data, encodeVarInt(0x12345)...) // flow ID
			data = append(data, encodeVarInt(0x6)...)     // length
			data = append(data, []byte("foobar")...)
			r := bytes.NewReader(data)
			frame, err := parseDatagramFrame(r, versionIETFFrames)
			Expect(err).ToNot(HaveOccurred())
			Expect(frame.FlowID).To(Equal(uint64(0x12345)))
	...
	Context("when writing", func() {
		It("writes a frame with flow ID and length", func() {
			f := &DatagramFrame{
				FlowID:         0x1234,
				DataLenPresent: true,
				Data:           []byte("foobar"),
		...
		It("writes a frame without flow ID", func() {
			f := &DatagramFrame{
				DataLenPresent: true,
				Data:           []byte("foobar"),
			}
	...
	Context("length", func() {
		It("has the right length for a frame with flow ID and length", func() {
			f := &DatagramFrame{
				FlowID:         0x1234,
				DataLenPresent: true,
				Data:           []byte("foobar"),
			}
			Expect(f.Length(versionIETFFrames)).To(Equal(1 + utils.VarIntLen(0x1234) + utils.VarIntLen(6) + 6))
		})

		It("has the right length for a frame without flow ID and length", func() {
			f := &DatagramFrame{
				DataLenPresent: true,
				Data:           []byte("foobar"),
			}
		...
		It("has the right length for a frame without flow ID and without length", func() {
	...
	Context("max data length", func() {
		...
		for _, fid := range []uint64{0, 1} {
			flowID := fid

			Context(fmt.Sprintf("for Flow ID %d", flowID), func() {
